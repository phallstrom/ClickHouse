...
private:
    void tryReplaceAndEqualsChainsWithConstant(QueryTreeNodePtr & node)
    {
        ...
        if (and_operands.size() == 1)
        {
            ...
            if (operand_type->lowCardinality())
            {
                ...
                assert(!function_type->isNullable() && function_type->equals(*removeLowCardinality(operand_type)));
                node = createCastFunction(std::move(and_operands[0]), function_type, getContext());
            }
            else
            {
                assert(!function_type->isNullable() && function_type->equals(*operand_type));
